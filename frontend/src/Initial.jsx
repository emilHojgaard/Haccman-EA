...
function Initial(props) {
    ...
    useEffect(() => {
        ...
        if (input1Value.trim() === '') {
            ...
            if (input2Value === '' || (!isNaN(input2Value) && input2Value >= 8 && input2Value <= 120)) {
                setAgeError('');
                setReady(true);
            } else {
    ...
    useEffect(() => {
        ...
        const handleKeyPress = (event) => {
            ...
            if (event.keyCode === 13 && ready) { // Down arrow key
                playSoundEffect('navigate');
    ...
    return (
        <div className='background'>
            ...
            <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'center', color: '#FFFADE', fontFamily: 'ARCADE_I', fontSize: 30, textShadow: '4px 4px 0px #A9345C' }}>
                {'An Arcade Game for Jailbreaking LLMs'}
            </div>

            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4vh', marginTop: '1vh' }}>

                {moveCounter === 0 && <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', color: '#FFFFFF', gap: '1vh' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', color: '#FFFFFF' }}>
                        {'>> Enter a username'}
                    </div>
                    <input autoFocus id="nameInput" ref={inputRef} value={input1Value} onChange={handle1Change} className='vaporwave-input' maxLength='6' />
                </div>}

                {moveCounter === 1 && <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', color: '#FFFFFF', gap: '1vh' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', color: '#FFFFFF' }}>
                        {'>> Enter your age'}
                    </div>
                    <input autoFocus id='ageInput' value={input2Value} onChange={handle2Change} className='vaporwave-input2' />
                    {ageError && <div style={{ color: 'red' }}>{ageError}</div>}
                </div>}


                {moveCounter === 2 && <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', color: '#FFFFFF' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', color: '#FFFFFF' }}>
                    ...
                    <div id="boxes" className='checkbox_gender'>
                        ...
                        />
                        <label htmlFor='female'> Female</label><input
                            type='radio'
                            id='male'
                            name='gender'
                            value='Male'
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', color: '#FFFFFF', fontSize: 14, marginBottom: 20, paddingTop: 20 }}>
